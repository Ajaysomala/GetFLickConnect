...
export function BrowsePage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-ink">
      ...
      <section className="pt-32 pb-10 border-b border-line relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          {filtered.length === 0 ? (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filtered.map((c, i) => (
                <Reveal key={c.id} delay={(i % 6) * 70} variant="scale">
                  <Link
                    to={`/browse/${c.id}`}
                    ...
                    <div className="p-5">
                      ...
                      <div className="flex flex-wrap gap-1.5 mb-4">
                        {c.specialties.slice(0, 3).map((s) => (
                          <span
                            key={s}
                            className="text-[0.68rem] bg-panel-raised text-text-faint px-2 py-0.5 rounded-full"
                          >
                            {s}
                          </span>
                        ))}
