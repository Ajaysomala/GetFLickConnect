...
export function NavBar() {
  ...
  return (
    ...
    >
          ...
          {authUser ? (
            <div className="relative">
              ...
              >
                ...
                <ChevronDown
                  size={14}
                  className={`text-text-faint transition-transform ${profileOpen ? 'rotate-180' : ''}`}
                />
              </button>
              {profileOpen && (
                <div
                  className="absolute right-0 top-full mt-2 w-48 rounded-2xl border border-line bg-panel shadow-2xl overflow-hidden z-50 animate-fade-up"
            ...
            <>
              <Link
                to="/login"
                className="text-sm font-medium text-text-dim hover:text-text px-4 py-2.5 transition-colors"
              >
                Log in
              </Link>
              <Link
      ...
      {open && (
        <div
          className="lg:hidden mt-2 mx-4 rounded-2xl border border-line bg-panel overflow-hidden animate-fade-up"
          style={{ animationDuration: '0.2s' }}
        >
          ...
          <div className="flex gap-3 p-4">
            {authUser ? (
              <button
                onClick={handleLogout}
                className="flex-1 text-center py-2.5 text-sm border border-danger/30 text-danger rounded-full"
              >
                Log out
              </button>
            ) : (
              <>
                <Link
                  to="/login"
                  onClick={() => setOpen(false)}
                  className="flex-1 text-center py-2.5 text-sm border border-line rounded-full text-text-dim"
                >
                  Log in
                </Link>
                <Link
                  to="/signup"
                  onClick={() => setOpen(false)}
                  className="flex-1 text-center py-2.5 text-sm text-ink rounded-full font-bold bg-signal"
                >
                  Get Started
                </Link>
              </>
